...
void ExitProgram(const char *message, char *outFilename);                  //Exit and display something
...
//I use global variables because otherwise I'll be passing the addresses of these variables around
//making it less readable
int GlobalDepth = 0;
int GlobalGenNodes = 0;
int GlobalExpNodes = 0;
double GlobalSecs = 0;
...
int main(int argc, char *argv[100]) {
    ...
    ExitProgram(EXIT_MSG, "output.txt");
    ...
    for(a = 0; a < 4; a++) {
        ...
        if((Loc[a][0] == Loc[4][0]) && (Loc[a][1] == Loc[4][1])) {
            ...
            //Pacman is vulnerable, Pacman dies
                ...
                if(Lives == -1) ExitProgram(END_MSG, "output.txt");
...
****************************************************************/
void ExitProgram(const char *message, char* outFilename) {
	...
	double ExpSec=0;
	ExpSec = GlobalExpNodes / GlobalSecs;
	...
	fprintf(outFile, "Propagation=%s\n", ppg);
	fprintf(outFile, "Budget=%d\n", budget);
	fprintf(outFile, "MaxDepth = %d\n", GlobalDepth);
	fprintf(outFile, "TotalGenerated = %d\n", GlobalGenNodes);
	fprintf(outFile, "TotalExpanded = %d\n", GlobalExpNodes);
	fprintf(outFile, "Time = %.3f seconds\n", GlobalSecs);
	fprintf(outFile, "Expanded/Second = %f\n", ExpSec);
...
void GetInput() {
    ...
    case 'q': case 'Q':                            //End program
        ExitProgram(QUIT_MSG, "output.txt");
        break;
